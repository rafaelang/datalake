...
SCRIPT_S3_URI = os.getenv(
    'SCRIPT_S3_URI',
    's3://vtex.datalake/scripts/create_stage_data_orders/create-stage-orders.py'
) 
LOG_DEST_S3_URI = os.getenv(
    'LOG_DEST_S3_URI',
    's3://aws-logs-282989224251-us-east-2/elasticmapreduce/create_stage_orders/clusters/'
)
...
DESTINATION_S3_URI_PREFIX = os.getenv(
    ...
    's3://vtex.datalake/stage_data/orders/'
)
DATASRC_S3_PREFIX = os.getenv(
    'DATASRC_S3_PREFIX',
    's3://vtex.datalake/raw_data/orders/parquet_stage'
)
...
def lambda_handler(event, context):
    ...
    cluster_id = emr_client.run_job_flow(
        Name='Create Stage Orders Routine (' + time.strftime("%Y/%m/%d %H:%M:%S") + ')',
        ...
        Steps=[
            ...
            {
                'Name': 'Run pyspark script for Create Stage Orders in Last Hour',
                'ActionOnFailure': 'CONTINUE',
                'HadoopJarStep': {
                    'Jar': 'command-runner.jar',
                    'Args': ['spark-submit', '--deploy-mode', 'client', HOME_HADOOP + SCRIPT_NAME,
                        '--s3-read-path', DATASRC_S3_PREFIX,
                        '--s3-destination-path', DESTINATION_S3_URI_PREFIX]
                }
            },
            {
